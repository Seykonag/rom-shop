...
create table products (price numeric(38,2), sale_price numeric(38,2), stock boolean not null, id bigint not null, developer varchar(255), model varchar(255), photo oid, title varchar(255), primary key (id));
...
create table sales (category_id bigint unique,sale integer not null, created timestamp(6), ended timestamp(6), id bigint not null, primary key (id));
...
alter table if exists products_comentaries add constraint product_comentaries_KF_comment_id foreign key (comment_id) references comments;
alter table if exists products_comentaries add constraint product_comentaries_KF_product_id foreign key (product_id) references products;
...
alter table if exists users add constraint users_KF_mark_id foreign key (mark_id) references marks;